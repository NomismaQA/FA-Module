...
// https://nomisma.cloud.testinsights.io/app/#!/module-collection/guid/61dca431-5948-4a18-945e-9d0c7456060e
@TestModellerModule(guid = "61dca431-5948-4a18-945e-9d0c7456060e")
public class DetailedIncomeStatementCommision extends BasePage
{
	public DetailedIncomeStatementCommision (WebDriver driver)
	{
		super(driver);
	}
	...
	private By CommissionElem = By.xpath("//TD[contains(text(),'Commission')]");
...
*/
     ...
     */
	public void Click_Commission()
	{
        
		WebElement elem = getWebElement(CommissionElem);

		if (elem == null) {
    		ExtentReportManager.failStepWithScreenshot(m_Driver, "Click_Commission", "Click_Commission failed. Unable to locate object: " + CommissionElem.toString());

    		TestModellerLogger.FailStepWithScreenshot(m_Driver, "Click_Commission", "Click_Commission failed. Unable to locate object: " + CommissionElem.toString());

			Assert.fail("Unable to locate object: " + CommissionElem.toString());
        }

		elem.click();
          	

		ExtentReportManager.passStep(m_Driver, "Click_Commission");

		TestModellerLogger.PassStep(m_Driver, "Click_Commission");
	}
}