...
// 		ClosePopup.ValidateAndPopUp(m_Driver);
     ...
     * @name Enter SearchAgentName
     */
 	public void Enter_SearchAgentNamebyRandom() throws InterruptedException
 	{
 		...
 		elem.sendKeys(selectedagent);
 		//Thread.sleep(150000);
 		
// 		ClosePopup.ValidateAndPopUp(m_Driver);
 		
 		
  		ExtentReportManager.passStep(m_Driver, "Enter_SearchAgentName " + selectedagent);

  		TestModellerLogger.PassStep(m_Driver, "Enter_SearchAgentName " + selectedagent);
 	}
	...
	public void get_randomAgent() 
	{
		
		List<WebElement> AgentOptions=m_Driver.findElements(By.xpath("//tr/td[1]"));
		
		GetRandomElementfromList randobj=new GetRandomElementfromList();
		WebElement randElement=randobj.getRandomElement(AgentOptions);
		
		
		
		selectedagent=randElement.getText();
